...
builder.Services.AddAuthentication(options =>
    ...
    .AddJwtBearer("Wso2Jwt", options =>
    {
        if (!wso2Enabled)
        {
            // Fallback to local validation when WSO2 is not yet enabled
            options.TokenValidationParameters = new TokenValidationParameters
            {
                ValidateIssuer           = true,
                ValidateAudience         = true,
...
// ── WSO2 Identity Service (SCIM2 + ROPC) ────────────────────────────────────
// Named HttpClient for WSO2 IS; certificate validation is configurable via
// Wso2:AllowInvalidCertificate for environments using self-signed certs.
builder.Services.AddHttpClient<IWso2IdentityService, Wso2IdentityService>("Wso2HttpClient", client =>
{
    var baseUrl = builder.Configuration["Wso2:BaseUrl"];
...
{
    var allowInvalidCert = builder.Configuration.GetValue<bool?>("Wso2:AllowInvalidCertificate") ?? false;
...
static bool LooksLikeWso2Token(string token, string? configuredIssuer)
{
    ...
    // WSO2 access tokens typically include 'azp' (authorised party)
