...
public class CounterRepository : ICounterRepository
{
    ...
    public async Task<CounterResponseDto> CreateCounterAsync(string name, int centerId, int? assignedOfficerUserId)
    {
        await using var conn = new MySqlConnection(_connectionString);
        await conn.OpenAsync();
        ...
        try
        {
            const string insertSql = @"
                INSERT INTO counters
                    (center_id, name, status, is_open, assigned_officer_user_id, created_at, updated_at)
                VALUES
                    (@CenterId, @Name, 'Closed', 0, @AssignedOfficerUserId, UTC_TIMESTAMP(), UTC_TIMESTAMP());
            ...
            {
                insertCmd.Parameters.AddWithValue("@CenterId", centerId);
                insertCmd.Parameters.AddWithValue("@Name", name.Trim());
                insertCmd.Parameters.AddWithValue("@AssignedOfficerUserId", (object?)assignedOfficerUserId ?? DBNull.Value);

                var scalar = await insertCmd.ExecuteScalarAsync();
                counterId = Convert.ToInt32(scalar);
            }
            ...
            const string selectSql = @"
                SELECT
                    ...
                    c.name,
                    c.center_id,
                    CONCAT('Center ', c.center_id) AS center_name,
                    CASE
                        WHEN COALESCE(c.is_open, 0) = 1 OR c.status = 'Open' THEN 1
                        ELSE 0
                    END AS is_open,
                    CASE
                        WHEN t.token_number REGEXP '^[0-9]+$' THEN CAST(t.token_number AS UNSIGNED)
                        ELSE NULL
            ...
            await using (var selectCmd = new MySqlCommand(selectSql, conn, transaction))
            {
                ...
                if (!await reader.ReadAsync())
                {
                    throw new KeyNotFoundException("Counter not found after creation.");
                }

                created = MapCounterResponse((MySqlDataReader)reader);
            }
    ...
    public async Task<List<CounterResponseDto>> GetCountersByCenterAsync(int centerId)
    {
        ...
        var counters = new List<CounterResponseDto>();
        await using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
        {
            counters.Add(MapCounterResponse((MySqlDataReader)reader));
        }
    ...
    public async Task<CounterResponseDto?> GetCounterByIdAsync(int counterId)
    {
        ...
        return MapCounterResponse((MySqlDataReader)reader);
    }

    public async Task<CounterResponseDto?> UpdateCounterStatusAsync(int counterId, bool isOpen, string? reason)
    {
        const string updateSql = @"
            UPDATE counters
            SET is_open = @IsOpen,
                status = CASE WHEN @IsOpen = 1 THEN 'Open' ELSE 'Closed' END,
                closed_reason = CASE WHEN @IsOpen = 1 THEN NULL ELSE @Reason END,
                updated_at = UTC_TIMESTAMP()
            WHERE counter_id = @CounterId";
        ...
        {
            updateCmd.Parameters.AddWithValue("@CounterId", counterId);
            updateCmd.Parameters.AddWithValue("@IsOpen", isOpen ? 1 : 0);
        ...
        return await GetCounterByIdAsync(counterId);
    }

    public async Task<bool> CounterExistsAsync(int counterId, int centerId)
    {
        const string sql = @"
            SELECT COUNT(1)
            FROM counters
    ...
    public async Task<bool> CounterNameExistsInCenterAsync(string name, int centerId)
    {
        const string sql = @"
            SELECT COUNT(1)
            FROM counters
    ...
    public async Task<bool> IsOfficerUserAsync(int userId)
    {
        const string sql = @"
            SELECT COUNT(1)
            FROM users
    ...
    public async Task<(int WaitingCount, bool HasCalledToken)> GetCounterQueueStateAsync(int counterId)
    {
        const string sql = @"
            SELECT
        ...
        var waitingCount = reader.IsDBNull(reader.GetOrdinal("waiting_count"))
            ? 0
            : reader.GetInt32(reader.GetOrdinal("waiting_count"));

        var calledCount = reader.IsDBNull(reader.GetOrdinal("called_count"))
            ? 0
            : reader.GetInt32(reader.GetOrdinal("called_count"));

        return (waitingCount, calledCount > 0);
    }
    ...
    private static CounterResponseDto MapCounterResponse(MySqlDataReader reader)
    {
        ...
        {
            CounterId = reader.GetInt32(reader.GetOrdinal("counter_id")),
            Name = reader.GetString(reader.GetOrdinal("name")),
            CenterId = reader.GetInt32(reader.GetOrdinal("center_id")),
            CenterName = reader.GetString(reader.GetOrdinal("center_name")),
            IsOpen = reader.GetInt32(reader.GetOrdinal("is_open")) == 1,
            CurrentTokenNumber = reader.IsDBNull(reader.GetOrdinal("current_token_number"))
                ? null
                : reader.GetInt32(reader.GetOrdinal("current_token_number")),
            AssignedOfficerUserId = reader.IsDBNull(reader.GetOrdinal("assigned_officer_user_id"))
                ? null
            ...
            AssignedOfficerName = reader.IsDBNull(reader.GetOrdinal("assigned_officer_name"))
                ? null
                : reader.GetString(reader.GetOrdinal("assigned_officer_name")),
            CreatedAt = reader.GetDateTime(reader.GetOrdinal("created_at"))
        };
    }
