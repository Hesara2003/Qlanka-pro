...
interface QueueRefreshSignals {
    latestCalledToken: unknown;
    latestStatusUpdate: unknown;
    latestReassignment: unknown;
}
...
export function useCounter(
    ...
    useEffect(() => {
        if (!queueRefreshSignals) return;

        if (
            !queueRefreshSignals.latestCalledToken
            && !queueRefreshSignals.latestStatusUpdate
            && !queueRefreshSignals.latestReassignment
        ) {
            return;
        }
