...
export default function LiveQueuePage() {
    ...
    const { latestCalledToken, latestStatusUpdate, latestReassignment, connectionStatus } = useQueueHub(centerIdNum || undefined);
    ...
    // Real-time queue update when token reassignment occurs for the same center.
    useEffect(() => {
        ...
        setQueue((prevQueue) => {
            ...
            const withoutReassigned = prevQueue.filter((token) => token.tokenId !== latestReassignment.tokenId);
            const existsInPrevious = withoutReassigned.length !== prevQueue.length;

            const appendedQueue = existsInPrevious
                ? withoutReassigned
                : [
                    ...withoutReassigned,
                    {
                        tokenId: latestReassignment.tokenId,
                        tokenNumber: latestReassignment.tokenNumber,
                        status: "Waiting",
                        position: withoutReassigned.length,
                        eta: null,
                    },
                ];

            return appendedQueue.map((token, index) => ({
                ...token,
                position: index,
            }));
        });

        // If the app knows the citizen's active token id, show a subtle personal notification.
        const activeTokenIdRaw = localStorage.getItem("active_token_id");
        const activeTokenId = activeTokenIdRaw ? Number(activeTokenIdRaw) : NaN;
        if (!Number.isNaN(activeTokenId) && activeTokenId === latestReassignment.tokenId) {
            setStatusToast(`Your token has been moved to Counter #${latestReassignment.targetCounterId}`);
        }

        // Background sync to ensure queue positions are fully consistent.
        void fetchQueue();
    }, [latestReassignment, centerIdNum, fetchQueue]);
