    ...
    <main className="w-full max-w-[1400px] mx-auto py-8 text-gray-900 selection:bg-[#78d64b]/30">
        ...
        <div className="flex flex-col md:flex-row items-end justify-between gap-8">
          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-[11px] font-semibold text-gray-400">Network directory</span>
            </div>
            <h1 className="text-5xl lg:text-7xl font-semibold tracking-tight mb-6 leading-[0.9] text-gray-900">
              Service Centers
            </h1>
            <p className="text-lg text-gray-500 font-normal tracking-tight leading-relaxed">
          ...
          <div className="flex items-center gap-4 shrink-0 pb-2">
            ...
            {user?.role === "admin" && (
              <button
                ...
                className="shrink-0 flex items-center gap-3 px-8 py-4 bg-gray-900 border border-transparent hover:bg-black text-white text-xs font-semibold rounded-3xl shadow-lg transition-all active:scale-95"
              >
      ...
      <div className="bg-white rounded-[2.5rem] p-3 shadow-premium border border-gray-100 mb-12 flex flex-col lg:flex-row gap-3 items-center justify-between sticky top-8 z-30 mx-1">
        ...
        <div className="relative w-full lg:flex-1">
          <div className="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none">
            <svg className="h-5 w-5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
          ...
          <input
            ...
            className="block w-full pl-14 pr-12 py-4 border-none bg-gray-50/50 hover:bg-gray-50 focus:bg-white rounded-3xl text-gray-900 text-sm font-normal placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#78d64b]/20 transition-all shadow-inner"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto pr-1">
          <div className="flex bg-gray-50/50 p-1.5 rounded-[2rem] border border-gray-100 shadow-inner">
            <button
              ...
              className={`px-6 py-2.5 text-xs font-semibold rounded-full transition-all ${filter === "all" ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
            >
              All hubs
            </button>
            <button
              onClick={() => { setFilter("available"); setCurrentPage(1); }}
              className={`px-6 py-2.5 text-xs font-semibold rounded-full transition-all ${filter === "available" ? "bg-white text-emerald-600 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
            >
              Available
            </button>
            <button
              onClick={() => { setFilter("unavailable"); setCurrentPage(1); }}
              className={`px-6 py-2.5 text-xs font-semibold rounded-full transition-all ${filter === "unavailable" ? "bg-white text-red-600 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
            >
              Closed
            </button>
          </div>
          ...
          <div className="relative flex items-center bg-gray-50/50 border border-gray-100 rounded-[2rem] px-2 shadow-inner group">
            <span className="pl-4 text-[10px] font-normal text-gray-300 transition-colors">Sort by</span>
            <select
              value={sortBy}
              onChange={(e) => {
              ...
              className="appearance-none bg-transparent border-none text-[11px] font-semibold text-gray-900 py-3 pl-3 pr-10 focus:outline-none focus:ring-0 cursor-pointer"
            >
            ...
            <svg className="w-4 h-4 text-gray-400 absolute right-4 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
          </div>
          ...
          >
            <svg className={`w-5 h-5 ${loading ? "animate-spin text-[#78d64b]" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
      ...
      {loading && filteredAndSortedCenters.length === 0 && (
        <div className="flex flex-col items-center justify-center p-32 bg-white rounded-[3.5rem] border border-gray-100 shadow-sm">
          ...
          <p className="text-[11px] font-semibold text-gray-400">Syncing network database...</p>
        </div>
      )}
      ...
      {error && !loading && (
        <div className="bg-red-50/50 border border-red-100 p-16 rounded-[4rem] text-center shadow-sm max-w-2xl mx-auto">
          <div className="w-20 h-20 bg-red-100 rounded-[2.5rem] flex items-center justify-center text-red-500 mx-auto mb-8 shadow-inner">
             <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          </div>
          <h2 className="text-3xl font-semibold text-gray-900 tracking-tighter mb-4">Critical sync error</h2>
          <p className="text-sm font-normal text-red-600 mb-10 max-w-sm mx-auto opacity-80">{error}</p>
          <button onClick={refresh} className="px-12 py-5 bg-red-500 text-white text-xs font-semibold rounded-3xl hover:bg-red-600 transition-all shadow-lg active:scale-95">
            Retry Connection
          </button>
        </div>
      )}
      ...
      {!loading && !error && filteredAndSortedCenters.length === 0 && (
        <div className="bg-white p-32 rounded-[4rem] border border-gray-100 shadow-premium text-center max-w-4xl mx-auto">
          <div className="w-24 h-24 bg-gray-50 rounded-[3rem] flex items-center justify-center text-gray-200 mx-auto mb-8">
             <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
          </div>
          <h3 className="text-3xl font-semibold text-gray-900 tracking-tight mb-3">Hub not found</h3>
          <p className="text-sm font-normal text-gray-400 max-w-sm mx-auto leading-relaxed">
            {searchQuery
              ? `Negative search results for segment "${searchQuery}".`
              : "No active service centers detected in current sector."}
          </p>
          {searchQuery && (
            <button onClick={() => setSearchQuery("")} className="mt-10 px-10 py-4 bg-gray-900 text-white text-[11px] font-semibold rounded-3xl hover:bg-black transition-all shadow-md">
              Reset Query
            </button>
          )}
        </div>
      ...
      {!loading && !error && totalPages > 1 && (
        <div className="mt-20 flex items-center justify-between bg-white px-8 py-5 rounded-[2.5rem] border border-gray-100 shadow-premium overflow-hidden relative">
          ...
          <button
            ...
            className="flex items-center gap-3 text-[11px] font-semibold text-gray-400 hover:text-gray-900 disabled:opacity-30 transition-all group"
          >
            <svg className="w-5 h-5 transition-transform group-hover:-translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
            Backward
          </button>
          ...
          <div className="flex items-center gap-2 hidden sm:flex">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`w-11 h-11 flex items-center justify-center rounded-2xl text-[11px] font-semibold transition-all relative ${currentPage === page
                    ? "bg-gray-900 text-[#78d64b] shadow-xl scale-110"
          ...
          <div className="sm:hidden text-[11px] font-normal text-gray-400 leading-none">
          ...
          >
            Forward
            <svg className="w-5 h-5 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          </button>
        </div>
      )}
    </main>
