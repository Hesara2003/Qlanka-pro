...
export default function OfficerDashboardPage() {
    ...
    const { latestStatusUpdatedToken, connectionStatus } = useQueueHub(officerCenterId);

    useEffect(() => {
        if (!latestStatusUpdatedToken || !calledToken) return;
        if (actionOutcome) return;
        if (latestStatusUpdatedToken.tokenId !== calledToken.tokenId) return;

        reset();
    }, [latestStatusUpdatedToken, calledToken, actionOutcome, reset]);
