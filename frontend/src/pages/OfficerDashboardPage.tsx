...
export default function OfficerDashboardPage() {
    ...
    const officerCenterId = userCenterId ?? calledToken?.centerId;
    ...
    const fetchWaitingTokens = useCallback(async () => {
        ...
        try {
            const tokens = await counterApi.getWaitingTokens(user.counterId);
    ...
    useEffect(() => {
        if (!latestCalledToken && !latestStatusUpdate) return;
        void fetchWaitingTokens();
    }, [latestCalledToken, latestStatusUpdate, fetchWaitingTokens]);

    useEffect(() => {
        if (!latestReassignment || user?.counterId == null) return;
        if (latestReassignment.centerId !== officerCenterId) return;
        ...
        if (currentCounterId === sourceCounterId) {
            ...
            window.setTimeout(() => {
                ...
                setWaitingTokens((prev) => {
                    const withoutToken = prev.filter((token) => token.tokenId !== latestReassignment.tokenId);
                    return withoutToken
                        .map((token, index) => ({
                            ...token,
        ...
        if (currentCounterId === targetCounterId) {
            ...
            setWaitingTokens((prev) => {
                const alreadyExists = prev.some((token) => token.tokenId === latestReassignment.tokenId);
                if (alreadyExists) {
                    return prev;
                }

                const maxQueuePosition = prev.reduce((max, token) => {
                    const position = token.queuePosition ?? 0;
                    return position > max ? position : max;
                }, 0);

                const appended: WaitingTokenDto = {
                    tokenId: latestReassignment.tokenId,
                    tokenNumber: latestReassignment.tokenNumber,
                    queuePosition: maxQueuePosition + 1,
                    issuedTime: latestReassignment.reassignedAt,
                    status: "Waiting",
                };
    ...
    }, [latestReassignment, user?.counterId, officerCenterId, counterNameMap, fetchWaitingTokens]);
    ...
    const handleReassignConfirm = useCallback(
        ...
        [selectedToken, reassignToken, counterNameMap, fetchWaitingTokens],
    );
