...
export default function ServiceCenterCard({ center }: ServiceCenterCardProps) {
  ...
  return (
    <div className="group bg-white rounded-[2.5rem] p-4 lg:p-6 shadow-sm hover:shadow-premium border border-gray-100 transition-all duration-500 flex flex-col lg:flex-row items-center gap-6 lg:gap-10 relative overflow-hidden">
      ...
      <div className="flex items-center gap-6 shrink-0 w-full lg:w-auto">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-gray-900 font-semibold text-xl shadow-inner transition-transform duration-500 group-hover:scale-105 shrink-0 ${isAvailable ? "bg-[#78d64b]" : "bg-gray-100 text-gray-400"}`}>
          {initial}
        </div>
        
        <div className="flex-1 min-w-0">
          <h3 className="text-xl font-semibold text-gray-900 tracking-tighter leading-none mb-2 truncate">
            {center.name}
          </h3>
          <p className="text-[11px] font-normal text-gray-400 flex items-center gap-2 truncate">
            <svg className="w-3.5 h-3.5 text-[#78d64b]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
      ...
      <div className="flex flex-1 items-center justify-between lg:justify-start gap-8 lg:gap-16 w-full lg:w-auto px-2 lg:px-0">
        <div className="flex flex-col gap-1.5">
          <span className="text-[10px] font-normal text-gray-400 leading-none">Schedule</span>
          <p className="text-[11px] font-semibold text-gray-900">{center.openingTime} - {center.closingTime}</p>
        </div>
        
        <div className="flex flex-col gap-1.5">
          <span className="text-[10px] font-normal text-gray-400 leading-none">Daily Load</span>
          <p className="text-[11px] font-semibold text-gray-900">{center.capacity} hubs</p>
        </div>
        
        <div className="flex flex-col gap-1.5">
          <span className="text-[10px] font-normal text-[#78d64b] leading-none">Latency (avg)</span>
          <div className="flex items-center gap-2">
            <p className="text-[11px] font-semibold text-gray-900">~{center.averageServiceTimeMinutes} min</p>
      ...
      <div className="flex items-center justify-between lg:justify-end gap-6 w-full lg:w-auto shrink-0 pr-2">
        {/* Status Badge */}
        <div className={`px-4 py-2 rounded-full text-[10px] font-semibold flex items-center gap-2 border shadow-sm transition-all ${isAvailable ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-red-50 text-red-600 border-red-100"}`}>
        ...
        {!isAdmin ? (
          <button
            ...
            className={`flex items-center justify-center gap-3 px-8 py-3.5 rounded-full font-semibold text-xs transition-all duration-300 active:scale-95 whitespace-nowrap ${isAvailable
          ...
          >
            ...
            {isAvailable && (
              <svg className="w-4 h-4 text-[#78d64b] transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
            )}
          </button>
        ) : (
