...
export default function WaitingTokensList({
    ...
    const [flashTokenIds, setFlashTokenIds] = useState<number[]>([]);
    ...
    useEffect(() => {
        ...
        if (latestCancellation.counterId !== 0 && latestCancellation.counterId !== currentCounterId) return;

        setFadingTokenId(latestCancellation.tokenId);
        const fadeTimeout = window.setTimeout(() => {
            setTokens((prev) => {
                const filtered = prev.filter((token) => token.tokenId !== latestCancellation.tokenId);
                return filtered.map((token, index) => ({
                    ...token,
                    queuePosition: index + 1,
                }));
        ...
        return () => window.clearTimeout(fadeTimeout);
    }, [latestCancellation, currentCounterId]);

    useEffect(() => {
        if (!latestQueueUpdate) return;
        if (latestQueueUpdate.counterId !== currentCounterId) return;

        const updatedTokenIds = (latestQueueUpdate.waitingTokens ?? []).map((token) => token.tokenId);
        setFlashTokenIds(updatedTokenIds);
    ...
    }, [latestQueueUpdate, currentCounterId]);
                    ...
                    <div className="flex flex-col gap-3">
                        {tokens.map((token) => (
                            <div
                                ...
                                } ${token.tokenId === highlightTokenId ? "bg-amber-500/10" : ""} ${flashTokenIds.includes(token.tokenId) ? "bg-[#78d64b]/10" : ""}`}
                            >
